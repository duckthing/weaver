...
-- Slightly edited to work better with LuaLS, still public domain.
...
---Returns the default separator for a platform
---@param pl Path.Platform
---@return "\\" | "/"
...
---Windows has various aliases, UNIX doesn't
---@param s string
---@return string?
...
---@return string? driveOrServerName
...
---If sep is nil or missing, the ending separator is returned (nil is returned if the path has no ending separator).
---If sep is true, '\\', '/', the path is returned with an ending separator added (true means use path's own separator
---if it has one and failing that, use dsep or the default platform separator).
---If sep is false or '' the path without its ending separator is returned.
---success is false if trying to add or remove the ending separator from an empty
---path (note that even when that happens, the path can still be concatenated directly to a relative path and result
---in a valid path).
---Multiple consecutive separators are treated as one in that they are returned together and are replaced together.
---@param s string
---@param pl Path.Platform?
---@param sep true | false | "/" | "\\" | nil # The separator to add to the end. 'nil' does nothing, while 'true' detects and adds one
---@param default_sep "/" | "\\" | nil # Used as default platform separator if autodetect fails
---@return "/" | "\\" | string
...
---The arg sep can be nil (detect), true (set to default_sep), false (set to default_sep but only if both \ and / are
---found in the path, i.e. unify), '\\' or '/' (set specifically), or nil when empty_names is explicitly false
---(collapse duplicate separators only). default_sep defaults to the platform separator.
---Unless empty_names is true, consecutive separators are collapsed into the first one.
---@param s string
---@param pl Path.Platform?
---@param sep true | false | "/" | "\\" | nil
---@param default_sep "/" | "\\" | nil
...
---Get/set a Windows long absolute path (one starting with \\?\C:\).
---If long is nil, returns whether the path is a long or short Windows absolute path (returns nil for all other kinds
---of paths).
---Otherwise it converts the path, in which case long can be true (convert to long path), false (convert to short path)
---or 'auto' (convert to long style if too long, or to short style if short enough).
---@param s string
---@param pl Path.Platform?
---@param long boolean?
...
---Returns the extension from path.nameext
---
---'a.txt' -> 'txt'
---'.bashrc' -> nil
---'a' -> nil
...
---Get the path without the last component and separator.
---If the path ends with a separator then the whole path without the separator is returned.
---Multiple consecutive separators are treated as one. Returns nil for '', '.', 'C:', '/', 'C:\\' and \\server\.
---Returns '.' for simple filenames.
---@param s string
---@param pl Path.Platform?
...
---Same as path.dir, except the end separator is included.
---'/home/user/file.txt' -> '/home/user/'
---'/home/user/' -> '/home/user/'
---@param s string
---@param pl Path.Platform?
---@return string? directory
function path.dirsep(s, pl)
	---@diagnostic disable-next-line: deprecated
	local result = path.dir(s, pl)
	if result then
		local r = path.endsep(result, pl, true)
		return r
	end
	return nil
end

---Returns the parent directory, with the end separator.
---'/home/user/file.txt' -> '/home/user/'
---'/home/user/' -> '/home/'
---@param s string
---@param pl Path.Platform?
---@return string? directory
function path.parentdirsep(s, pl)
	local result = path.endsep(s, pl, false)
	---@diagnostic disable-next-line: deprecated
	result = path.dir(result, pl) or ""
	if result then
		local r = path.endsep(result, pl, true)
		return r
	end
...
---Combine two paths if possible (return nil, err if not).
---Supported combinations are between anything except dev_alias and rel paths, between abs_nodrive and rel_drive,
---and between rel_drive and abs or abs_long.
---When the paths can only be combined in one way, paths can be given in any order.
---The separator with which paths are combined is either sep or if sep is nil it's detected and if that fails dsep
---or the default separator is used.
---@param s1 string
---@param s2 string
---@param pl Path.Platform?
---@param sep "/" | "\\" 
---@param default_sep any
---@return string? path
---@return string? err
