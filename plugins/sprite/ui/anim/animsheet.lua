...
---@class AnimTimeline.Sheet: Plan.Container
local SheetTimeline = Plan.Container:extend()
local celSize = 26
...
function SheetTimeline:new(rules)
	SheetTimeline.super.new(self, rules)
	---@type Sprite?
	self.sprite = nil
	---@type integer
	self.scrollX = 0
...
function SheetTimeline:updateScroll()
...
function SheetTimeline:mousemoved(newX, newY, changeX, changeY)
...
function SheetTimeline:mousepressed(_, _, button)
...
function SheetTimeline:mousereleased(mx, my, button)
...
function SheetTimeline:pointerentered()
	self.hovering = true
	self.shouldUpdateSB = true
end

function SheetTimeline:pointerexited()
	self.hovering = false
	self.shouldUpdateSB = true
	self.hoveringIndexX = 0
	self.hoveringIndexY = 0
...
function SheetTimeline:wheelmoved(x, y)
...
function SheetTimeline:recalculateBoundaries()
...
function SheetTimeline:updateSpriteBatch()
...
function SheetTimeline:refresh()
	SheetTimeline.super.refresh(self)
	self:updateScroll()
end

---@param sprite Sprite
function SheetTimeline:onSpriteSelected(sprite)
	self.sprite = sprite
...
function SheetTimeline:onSpriteDeselected()
...
function SheetTimeline:onLayerSelected(selectedLayer)
	self.shouldUpdateSB = true
...
function SheetTimeline:onFrameSelected(selectedFrame)
	self.shouldUpdateSB = true
...
function SheetTimeline:draw()
	...
	SheetTimeline.super.draw(self)
	love.graphics.setScissor(ox, oy, ow, oh)
end

return SheetTimeline
